...
def create_lv2_file():
    ...
    init_upp_times=[]
    init_flare_signifs=[]
    with open("initial_flare_candidates_20220104.csv","r") as INFILE:
        for lines in INFILE:
            init_i_prime.append(int(lines.split(",")[0]))
            initial_TID.append(int(lines.split(",")[1]))
            init_Prot.append(float(lines.split(",")[2]))
            init_Psnr.append(float(lines.split(",")[3]))
            init_low_times.append(float(lines.split(",")[4]))
            init_flare_times.append(float(lines.split(",")[5]))
            init_upp_times.append(float(lines.split(",")[6]))
            init_flare_signifs.append(float(lines.split(",")[7]))
    ...
    which_tic=[]
    which_desig=[]
    with open("which_stars_all_FPs.csv","r") as INFILE:
        next(INFILE)
        for lines in INFILE:    
            which_i_prime.append(int(lines.split(",")[0]))
            which_tic.append(int(lines.split(",")[1]))
            which_desig.append(str(lines.split(",")[2].replace(" ","").rstrip("\n")))
    which_i_prime=np.array(which_i_prime)
    which_tic=np.array(which_tic)
    which_desig=np.array(which_desig)

    count=0
    
    with open("initial_flare_candidates_20220104.csv","r") as INFILE_2:
        for lines in INFILE_2:
            TEMP_I = int(lines.split(",")[0])
            DESIG_I = which_desig[which_i_prime==TEMP_I][0]
            if DESIG_I=="n":
                continue
            with open("lv2_initial_flare_candidates_20220104.csv","a") as OUTFILE:
                OUTFILE.write(str(count)+","+lines)
            count+=1
    return

#head = i_prime, TICID, Prot, Psnr, low_times, flare_times, upp_times, flare_signifs

#uniq_init_i_prime=[] #does not get assigned until pass 2
init_i_prime=[]
initial_TID=[]
init_Prot=[]
init_Psnr=[]
...
init_upp_times=[]
init_flare_signifs=[]
with open("initial_tess_flare_candidates.csv","r") as INFILE:
    for lines in INFILE:
        init_i_prime.append(int(lines.split(",")[0]))
        initial_TID.append(int(lines.split(",")[1]))
        init_Prot.append(float(lines.split(",")[2]))
...
#grab all LC out of directory
successes = glob.glob("./tess_lcvs/tess*_lc.fits")
successes = np.sort(np.unique(np.array([int(x.split("-")[2]) for x in successes])))

"""
# Check for desig-reduced initial_flare_candidates_YYYYMMDD.csv
level_two_file = glob.glob("./lv2_initial_flare_candidates_*.csv")
if len(level_two_file)==0:
    create_lv2_file()
else:
    os.system("rm lv2_initial_flare_candidates_20220104*.csv")
    create_lv2_file()
"""
...
for i_prime in range(len(successes)):
    ...
    if TID not in np.unique(initial_TID):
        continue #skip stars with no flare detections for now
    ...
    P_ROT = init_Prot[initial_TID==TID][0]
    P_SNR = init_Psnr[initial_TID==TID][0]
    ...
    orig_y_vals = copy.deepcopy(y_vals)
    ...
    for u in range(len(target_flpeaks)):
        diff = np.absolute(target_flpeaks[u] - x_vals)
        AMPL = y_vals[list(diff).index(np.min(diff))]
        assoc_ampl.append(AMPL)
    assoc_ampl=np.array(assoc_ampl)
    ...
    plt.title("Star "+str(i_prime)+",  TIC-"+str(TID)+",  PROT="+str(PROT)+" d")

    for u in range(len(target_flpeaks)):
        plt.axvspan(target_flstarts[u], target_flstops[u], facecolor="grey",zorder=0.01)
    
    plt.plot(x_vals, y_vals, marker="+", ls="none", color="black",zorder=0.1)
    plt.plot(target_flpeaks, assoc_ampl, marker="o", ls="none", color="red",zorder=0.99)
    ...
    #plt.title(i_prime)
    plt.plot(x_vals, orig_y_vals, marker="+",ls="none",color="black")
